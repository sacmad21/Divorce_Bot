...
export default function ChatPage() {
  ...
  const [collectionName, setCollectionName] = useState("divorce");
  ...
  const [userId, setUserId] = useState(() => storedId || urlId || "");
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      const fullResponse = botResponseText;
  ...
  const exportChat = () => {
    const chatText = messages
      .map(
        (msg) =>
          `[${msg.timestamp}] ${msg.sender === "user" ? "You" : "AI"}: ${
            msg.text
          }`
      )
      .join("\n\n");
  ...
  const saveConvo = async (username, sessionId, messages, collectionName) => {
    ...
    try {
      // Ensure you have the userId from state or props
      //console.log(userId);
      ...
      const response = await axios.post("/api/conversations", {
        username, // ✅ Send userId in the request body
        sessionId,
        messages,
        collectionName,
      });
      ...
      if (response.status === 200) {
        ...
        router.push(`/conversations?username=${username}`); // ✅ Redirect with userId
      } else {
  ...
  const fetchConvo = async (username, router) => {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        // Redirect to conversations page with userId in query params
  ...
  return (
    <div
      className={`flex flex-col min-h-[100dvh] ${baseTheme.background} ${baseTheme.text} ${baseTheme.transition} font-plus-jakarta`}
    >
  ...
  } rounded-md shadow-xl border ${baseTheme.border} font-plus-jakarta`}
  style={{
    height: "90vh",
    maxHeight: "90vh",
    width: "100%",
...
>
  {/* Header */}
  <div className="sticky top-0 z-10 backdrop-blur-md bg-opacity-95 bg-[#4267B2] text-white py-4 px-6 shadow-md flex items-center justify-between rounded-t-xl">
    <div className="flex items-center space-x-3">
      <div className="bg-white bg-opacity-20 p-2 rounded-full">
        <Bookmark size={20} className="text-white" />
      </div>
      <span className="font-bold text-lg font-outfit">Saved Bookmarks</span>
    </div>
    <button
  ...
  <div className={`p-4 border-b ${baseTheme.border}`}>
    ...
    <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 mt-1">
      {["hour", "day", "week", "all"].map((time) => {
        const isActive = filter === time;
        const commonClasses = `p-2 text-xs rounded-md font-dm-sans ${baseTheme.transition} ${baseTheme.buttonActive}`;
        const activeClasses = "bg-[#4267B2] text-white";
        const inactiveDark = "bg-gray-700 text-gray-300";
        const inactiveLight = "bg-gray-200 text-gray-700";
  ...
  >
    {bookmarksLoading ? (
        ...
        {bookmarks.map((b) => (
          ...
          >
            <div className="flex justify-between items-start mb-2">
              <div
                className={`text-xs font-medium flex items-center font-montserrat ${
      ...
      <div
  className={`flex-1 overflow-y-auto auto-scrollbar pb-2 p-4 space-y-4 ${baseTheme.background} ${baseTheme.transition}`}
...
>
        {messages.length === 0 ? (
          <div className="h-full flex flex-col items-center justify-center text-center px-4">
            ...
            >
              Ask questions, get information. Bookmark and Save response.
            </p>
              ...
              {startingQuestions.map((question, index) => (
                <button
                  ...
                  className={`text-left p-4 rounded-lg border ${
                    ...
                    baseTheme.card
                  } hover:shadow-md transition flex items-start ${
                  ...
                  <div className="flex items-center justify-between mb-1">
                    ...
                    <div className="flex items-center space-x-1">
                      ...
                      {message.sender === "bot" && (
                        <Button
                          icon={
                            bookmarkedMessages.has(message.text) ? (
                              <IoMdBookmark size={14} />
                            ) : (
                              <Bookmark size={14} />
                            )
                          }
      ...
      <div className="sticky bottom-0 z-10 w-full bg-inherit border-t p-3 backdrop-blur-sm shadow-md">
      <form
        onSubmit={(e) => {
          e.preventDefault();
          if (input.trim()) {
  ...
  className={`w-full resize-none text-sm px-4 py-2.5 rounded-md border focus:ring-2 focus:ring-blue-100 focus:outline-none transition-all duration-150 ${
    darkMode
...
/>
        ...
        <button
          ...
          className="p-3 bg-[#4267B2] text-white rounded-full hover:bg-[#365899] transition disabled:opacity-50 disabled:cursor-not-allowed"
        >
          <Send size={18} />
        </button>
      </form>
